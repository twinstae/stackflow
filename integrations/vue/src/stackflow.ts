...
export type StackComponentType = DefineComponent<{
	initialContext?: any;
}>;

type StackflowPluginsEntry<T extends BaseActivities> =
	| StackflowVuePlugin<T>
...
export function stackflow<T extends BaseActivities>(
	...
	const plugins = (options.plugins ?? [])
		.flat(Infinity as 0)
		.map((p) => p as StackflowVuePlugin<T>);
	...
	const activityComponentMap = Object.entries(options.activities).reduce(
		(acc, [key, Activity]) => ({
			...acc,
			[key]:
				"component" in Activity ? Activity.component : Activity,
		}),
		{} as {
	...
	const Stack = defineComponent({
		setup(props: {
			...
			return { pluginInstances, coreStore, activityComponentMap };
		},
		components: {
			PluginsProvider, CoreProvider, MainRenderer
		},
		template: `
